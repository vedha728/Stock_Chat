...
def main():
    ...
    cipla_df = df[df['Ticker'] == 'CIPLA.NS']
    
    total = len(cipla_df)
    available = cipla_df['Sentiment_Available'].sum()
    missing = total - available
    missing_pct = (missing / total) * 100 if total > 0 else 0
    
    safe_print(f"Cipla Total Rows: {total}")
    safe_print(f"Cipla Sentiment Available: {available} ({100 - missing_pct:.2f}%)")
    safe_print(f"Cipla Sentiment Missing: {missing} ({missing_pct:.2f}%)")
    
    missing_dates = cipla_df[cipla_df['Sentiment_Available'] == 0]['Date'].tolist()
    safe_print(f"\nFound {len(missing_dates)} dates with 0 headlines after filtering.")
    
    # Pick 6 sample dates spread out across the range
    if len(missing_dates) >= 6:
        indices = [0, len(missing_dates)//5, 2*len(missing_dates)//5, 3*len(missing_dates)//5, 4*len(missing_dates)//5, len(missing_dates)-1]
        ...
        try:
            ...
            for idx, item in enumerate(items, 1):
                ...
                if not is_date_match:
                    ...
                    status = f"FILTERED (Alias Mismatch: title does not contain 'cipla')"
                    filtered_by_alias += 1
                else:
                    kept += 1
